...
const isFav = (id) => {
    ...
    return ((favsObj === null || favsObj === void 0 ? void 0 : favsObj[id]) instanceof Object) ? true : false;
};
const favHandler = (event) => {
    ...
    if (event.target instanceof HTMLElement && favsObj instanceof Object) {
        ...
        if (fav.id in favsObj) {
            ...
            favsObj = Object.assign(Object.assign({}, favsObj), { [fav.id]: {
                    id: fav.id,
...
export function renderSearchResult(place, isFav) {
    return `
      <li class="result">
        <div class="result-container">
          <div class="result-img-container">
...
export function renderSearchResultsBlock(resultsArr) {
    renderBlock('search-results-block', `
    <div class="search-results-header">
        <p>Результаты поиска</p>
        <div class="search-results-filter">
    ...
    <ul class="results-list">      
  ${resultsArr instanceof (Array) ?
        resultsArr.map(p => renderSearchResult(p, isFav(p.id))).join('') :
        '<h1>Something is wrong with recieved data</h1>'}
    </ul>
    `);
    document.querySelectorAll('.favorites').forEach(heart => heart.addEventListener('click', (e) => favHandler(e)));
}
